from scipy.stats import entropy  # for testing results only!
...
def print_encoded_first_name(results_dict):
    while True:
        try:
            ...
            print("did you enter the name in hebrew? - please try to run again")
        else:
            break
    print(f"in huffman code:{s}")
    print(entropy([ int(x) for x in s ]))
...
def huffman_on_name(first_name1):
    ...
    freq, probabilities = fill_probabilities(freq)
...
def run():
    hebrew_letters_freq = {'א': 0.0634, 'ב': 0.0474, 'ג': 0.013, 'ד': 0.0259, 'ה': 0.1087, 'ו': 0.1038, 'ז': 0.0133,
                           'ח': 0.0248,
                           'ט': 0.0124,
                           'י': 0.1106, 'כ': 0.027 + 0.0081, 'ל': 0.0739, 'מ': 0.0303 + 0.0459, 'נ': 0.011 + 0.0286,
                           'ס': 0.0148,
                           'ע': 0.0323,
                           'פ': 0.0027 + 0.0169, 'צ': 0.0012 + 0.0124, 'ק': 0.0214, 'ר': 0.0561, 'ש': 0.0441,
                           'ת': 0.0501}
    hebrew_letters_freq, probabilities = fill_probabilities(hebrew_letters_freq)
    huffman_code = compute_code(probabilities)
    print('אות | Huffman code ')
    print('----------------------')
    results_dict = {}
    for idx, char in enumerate(hebrew_letters_freq):
        if huffman_code [ idx ] == '':
